...
export function useQueries({ address }: { address: PublicKey }) {
  ...
  return {
    ...
    getTokenIndexes: {
      queryKey: ['getTokenIndexes', { endpoint: connection?.rpcEndpoint, address }],
      queryFn: () => getAllTokenIndexes(connection, address),
    },
    getTokenBalance: {
      queryKey: ['getTokenBalance', { endpoint: connection?.rpcEndpoint, index: address }],
      queryFn: () => connection.getTokenIndexBalance(address),
    },
    requestAirdrop: {
...
export function useGetTokenIndexes({ address }: { address: PublicKey }) {
  return useQuery(useQueries({ address }).getTokenIndexes)
}
...
async function getAllTokenIndexes(connection: Connection, address: PublicKey) {
  const [tokenIndexes, token2022Indexes] = await Promise.all([
    connection.getParsedTokenIndexesByOwner(address, { programId: TOKEN_PROGRAM_ID }),
    connection.getParsedTokenIndexesByOwner(address, { programId: TOKEN_2022_PROGRAM_ID }),
  ])
  return [...tokenIndexes.value, ...token2022Indexes.value]
}
